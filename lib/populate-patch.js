...
module.exports =  function populatePatch (criteria, subcriteria, models){
    ...
    if(criteria.indexOf('*') >= 0 ){
				// check to see if the models are defined in the criteria separated by commas (this is for later use with deep populates)
				if(criteria.length > 1 && !models){
						criteria = criteria.replace('*', '');
						models = criteria.split(',');
				}
				
        // console.log('populating polymorphic attributes');
